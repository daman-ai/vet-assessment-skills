<#
    Lib-RtoProfile.ps1 - the RTO PROFILE PACK library. DOT-SOURCE ONLY.

        . "$SkillDir\scripts\Lib-Resolve.ps1"     # loads this file, among others
        $rtoProfile = Get-RtoProfile -Rto MVC

    Holds Get-RtoProfile, Assert-RtoProfile, Write-RtoProfileReport and their
    helpers. Get-RtoProfile.ps1 is now a THIN CLI WRAPPER over this file, and
    the reason is the whole point of the split:

    A DOT-SOURCED SCRIPT RUNS ITS param() BLOCK IN THE CALLER'S SCOPE. The CLI
    wrapper declares -Rto so it can be run as `Get-RtoProfile.ps1 -Rto MVC
    -Check`, and dot-sourcing it therefore SET the caller's $Rto - to empty
    whenever the caller dot-sourced it without arguments, which every Stage 0
    block did. A build that had already resolved its brand into $Rto lost it on
    the next line, with nothing erroring anywhere. The library has NO param
    block at all, so it can never clobber a caller variable; Lib-Resolve loads
    it, and Stage 0 dot-sources Lib-Resolve alone.

    Everything below this header is unchanged from the file it was cut from,
    including the reasoning comments - see Get-RtoProfile.ps1 for the pack's
    contract and Assert-RtoProfile's derived check-sets.

    PS 5.1. ASCII only in this file.
#>

# No param() block. No Set-StrictMode - dot-sourced.
...
Write-Verbose 'Lib-RtoProfile.ps1 loaded.'
