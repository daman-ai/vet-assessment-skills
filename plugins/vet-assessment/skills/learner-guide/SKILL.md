...
| `references/visuals.md` section 9, and the `docx-images` sub-skill | Stage 7b |
...
| Route A generation and the image review (7b-i, launched at the end of 3b) | 7b placement |
...
```powershell
. "$SkillDir\scripts\Lib-Resolve.ps1"       # ONE line. It loads the shared OOXML library AND
                                            # this skill's own - Lib-RtoProfile (Get-RtoProfile,
                                            # Assert-RtoProfile), Set-ResourceBrand, the renderers
                                            # and the rule sets - each through a named, throwing
                                            # loop, so a missing library file fails HERE by name.
...
**Do not dot-source `Get-RtoProfile.ps1`.** It is now a thin CLI wrapper over `Lib-RtoProfile.ps1`, and it keeps its `param()` block so it can still be run as `Get-RtoProfile.ps1 -Rto MVC -Check`. Dot-sourcing it therefore still sets your `$Rto` - to empty, whenever you dot-source it without arguments, which every previous Stage 0 block did. `Lib-RtoProfile.ps1` has no `param()` block at all and cannot clobber anything; `Lib-Resolve.ps1` loads it, so one line is the whole of it.
...
- **`Assert-AllowListDiscipline`** - **Status: NOT YET IMPLEMENTED** - performed today by: `Get-GateAllowList` in `scripts\Lib-GateCommon.ps1`, which refuses an entry with no written reason, for the gates that read their allow-lists through it; nothing sweeps every gate for an allow-list held somewhere else. Specified: every allow-list entry lives in the **versioned registry beside the rule it weakens**, never as a script parameter default, carries a **written reason**, and is surfaced to the audit as evidence. A build-local `Check-`/`Test-` script that is not a copy of a skill script must record why a new gate was needed.
...
**Run the whole of the above through `scripts\Invoke-Stage0.ps1`, and read the file it writes.**

```powershell
& "$SkillDir\scripts\Invoke-Stage0.ps1" -BuildDir $out -Rto $brand -UnitCode $code
$s0 = Get-Content (Join-Path $out '0-results.json') -Raw | ConvertFrom-Json
Add-StageRecord -BuildDir $out -Stage '0' -Name 'Pre-flight' -Status $(if ($s0.verdict -eq 'PASS') { 'pass' } else { 'fail' }) `
                -Partial $s0.partial
```

Stage 0 is the stage that decides whether the gates are trustworthy at all, and until this runner existed it was the only machine-gated stage with **no results file**. On the reference build it recorded a CommandNotFound throw for the palette check and a non-zero exit from the fixtures gate, and the ledger record said `pass` with a note asserting the opposite of both; nothing on disk could contradict it.

`0-results.json` is written in the same shape as `3c-results.json`, so one reader serves stages 0, 1, 2, 3c, 4 and 7c. Its `verdict` is **computed from what each member did** - a member that throws is FAIL with the exception as its reason, a member that exits non-zero is FAIL with the code recorded, and a member nobody has built is NOT RUN and is named in `partial[]`, never PASS.

**The member set is derived from the `references/gates.md` stage table** - every row whose Stage column is `0` or `S0-RTO`, with that row's own Blocks column deciding whether it can fail the stage. A documented Stage 0 gate the runner has no binding for is a REFUSAL naming the row, so the documentation and the runner move together. The three bullets above marked NOT YET IMPLEMENTED are read out of gates.md **by that marker**, recorded NOT RUN with it as the reason, and named in `partial[]`.

The endpoint probe stays **non-blocking by declaration**: gates.md says Blocks=no, the runner reads that column, and the probe's exit code is recorded in full and kept out of `failed[]`. Its exit code is captured on the statement after the call, never later.

The fixtures member runs `-StaticOnly` (with `-SkillDir` and `-ResultDir`). Stage 0 asserts the static arms and then names, in `partial[]`, every Blocks=yes gate the newest `gate-fixtures.<hash>.json` did not prove. The plant channel is a background job keyed on the scripts hash; **it need not finish before Stage 0**.

**A Stage 0 verdict distinguishes three things, and they are not interchangeable:**

- **`failed[]`** - a blocking member that threw, exited non-zero, was refused, or was NOT RUN for any reason other than being unbuilt. These fail the stage.
- **`notImplemented[]`** - a blocking member whose gates.md row is marked NOT YET IMPLEMENTED. Recorded NOT RUN, named in `partial[]`, printed in the summary with its count, and it can never read as PASS - but it does not fail the stage, because nobody has built it and the roadmap says not to. Failing forever on it would make the verdict carry no information.
- **`partial[]`** - every named thing that did not happen: the unbuilt members, every Blocks=yes gate the newest fixtures report did not prove, and any member recorded NOT RUN.

Measured on the current tree: 3 unbuilt blocking members (`Assert-RendererContract`, `Assert-DownstreamPalette`, `Assert-LongStageOutputContract`) and 20 Blocks=yes gates unproven because only a `-StaticOnly` fixtures report exists.

Pre-flight also **declares two things every later stage is held to**:

- **The long-stage output contract** (`Assert-LongStageOutputContract`). **Status: NOT YET IMPLEMENTED** - performed today by: nobody; the contract is stated in this section and enforced by the operator. Every long-running judgement stage creates its output file and writes its **header and scope section before analysis begins**, then appends each section as it completes, under a heartbeat, and on restart resumes by reading what it already wrote. On one build the third audit round ran three times: two runs died mid-analysis having written nothing, and the run that survived did so only because it was restarted with instructions to write the report first.
- **The mandated-sweep list, and the render and placement sets.** They are two classes, not one, and `$script:LedgerStages` owns both: a stage with `Render` true (`4` and `7`) assembles both artefacts from a fresh template, a stage with `Placement` true (`7b` and `7c`) changes the page without changing the prose. Read the two sets from the table, never from a copy. Every stage record must enumerate which mandated sweeps actually ran.
...
# Take the span. utc is only an APPEND time and no rule reads it any more:
...
# A stage with a results file needs neither -Started/-Ended nor a note: both are
# DERIVED from the file, and free text goes to -OperatorNote.
Add-StageRecord -BuildDir $out -Stage '4' -Name 'Render + full gate set' -Status pass `
                -Partial $gr.Partial -OperatorNote 'pack question list not yet parsed; re-run at 7c'
```

**Spans, and why the append time is not one.** `Add-StageRecord` writes `started`, `ended`, `durationSeconds` and `spanKnown` beside `utc`. `-Started`/`-Ended` are ISO 8601 UTC, sub-second; omitted, they are derived from the stage's results file (`startedAt`, `ranAt`, `wallClockSeconds`). With neither source the record carries `spanKnown = false` and `Test-StageLedger` names it. **A blocking stage recorded with `started` equal to `ended` is refused at the point of writing.** Two records of DIFFERENT stages may share an ended-second only when both spans are known and do not overlap - on one build 21 of 33 records sat within 0.1 s of their predecessor, which is a list of intentions, not a record of what happened. Legacy records with an append time and no span are REPORTED, never accepted as spans.

**Every stage that `$script:LedgerStages` marks Required must have a record.** There is now ONE ordered stage table in `scripts\Stage-Ledger.ps1` - `$script:LedgerStages`, whose rows carry `Key, Title, Required, Blocking, Conditional, Terminal, Render, Placement, StaleAfterRender, StaleAfterPlacement, PostPlacementRead, Verdict, Artwork, Script` - and every older list name (`$script:LedgerRequired`, `$script:LedgerBlocking`, `$script:LedgerOrder`, `$script:LedgerRenders`, `$script:LedgerPlacements`, ...) is a DERIVED VIEW of it. Read the table from the script; never transcribe it here. A hand-copied stage list is exactly how six blocking stages came to be enforced by nothing, and five hand-listed arrays are five copies to keep in step.

**Stage 7 is conditional, and this table is the single owner of that rule.** It is required when a Stage 6 or 7d record carries `round > 0` or a verdict below the best one in the closed vocabulary (read from `Merge-AuditFindings.ps1` by AST), or when a Stage 6b record carries a non-empty work order. It blocks whenever it is present, and it is never in the unconditional required set: a build with no findings needs no remediation round.

**The blocking set now includes 0, 1, 2, 3b, 7 and 8.**

**New and changed parameters.** `Add-StageRecord` gains `-Started`, `-Ended`, `-Machine` (a sha256 from a stage whose own writer produced the evidence, e.g. `Merge-AuditFindings`; refused on a stage that has a results file, and requires `-Round` >= 1), `-OperatorNote` (free text, where the note itself is generated), `-ClearancesApplied`, `-FigureSheet` and `-SpineDir`; `-Round` may no longer go backwards for a stage. `Test-StageLedger` gains `-InProgress` (excludes ONLY the terminal stage 8) and `-SpineDir`, and returns `.Reported`, `.Machine`, `.Stage7`, `.Required`, `.InProgress` and `.GeneratedNote` beside `.Ok`/`.Problems` - **the Stage 8 note is generated from `.GeneratedNote`, not typed.** `Test-FigureSheetCurrent` gains `-SpineDir` and `-Records`, and returns `.BandVerdict` and `.Extracts`. The command line gains `-InProgress`, `-SpineDir` and `-SelfTest`:

```
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Stage-Ledger.ps1 -BuildDir <build> -Check [-InProgress] [-SpineDir <dir>]
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\Stage-Ledger.ps1 -SelfTest
```

**Exit codes are the house convention: 0 pass, 1 finding, 2 refused (input missing), 4 self-test failed.** The old `exit 6` for a ledger finding is gone.

`skipped` is an honest status and it is allowed to be WRITTEN; it will not pass Stage 8 for any stage that is in the pipeline, which is the point. **`n-a` is accepted at delivery only on the artwork stages (`7b-i`, `7b`), and only where a Stage 2 record in the same ledger records the artwork decision as no-go.** Everywhere else an `n-a` is a hard problem naming the stage: three blocking stages were once recorded `n-a` plus a note and passed delivery. Both statuses still cost a written `-Note`.

**There are four statuses and there is no fifth.** A dispositioned failure is recorded as a `pass` whose `-ClearancesApplied` entries are printed by the ledger report every time it runs.
...
**Every write goes through `New-SpineWriter`, which validates against the compiled schema and REFUSES the write**, returning the violation to the agent to fix in-loop. **Status: NOT YET IMPLEMENTED** - performed today by: `scripts\Test-SpineRead.ps1`, which reports UNREAD and MISSING AFTER the write rather than refusing it, and `scripts\Test-SubSection.ps1 -File <spine file>`, the agent's own pre-return check; agents write spine JSON directly and nothing refuses a write. The `kind` and dangling-token classes are performed by nobody at write time. The refusal set is deliberately narrow - **exact, locally fixable violations only**:
...
**`Assert-SpecRenderable`, exact arm, at write time**: a visual spec must declare its `kind`, and a spec whose node count exceeds the renderer's box cap is refused where it is written. **Status: NOT YET IMPLEMENTED** - performed today by: `scripts\Assert-SpecRenderable.ps1`'s WHOLE-SPINE arm at 3c, which takes `-BuildDir` and declares no per-file write-time mode, so the same defect is found one stage later rather than refused where it is written.
...
Emit each as a prompt block with `GImagePrompt` at the exact placement. The guide is built with **prompts on the page, not pictures**; 7b turns them into artwork.
...
**`Run-SpineGates.ps1` is the only way this stage is run or recorded** (landed 3 Sep 2026). It introspects every member's parameters and passes only what that copy declares; a member that is absent, refuses a missing input, throws or exceeds `-TimeoutMinutes` is a FAIL naming the reason, never a skip; `-Only` prints a PARTIAL RUN banner and exits 3, so a subset can never stand for the band; and the figure sheet is cut in a third phase only after every blocking member passed, so 3d never reads a sheet from a failed spine. A missing `unit_extract.md` refuses the whole run, because the leakage sweep needs it. The 3c ledger note records the members that ran from `3c-results.json`, and a band pass is a pass of exactly those members.

**The member list is not repeated here. Band membership is DERIVED** from the roster in `scripts\Run-SpineGates.ps1` plus every script whose own `# GATE: stages=` header names the stage, which the runner reads at run time - so the headers on disk are the register. A roster member whose header omits `3c` is REFUSED ("a member cannot leave the band by editing its own header"); a script with no header yet gets a printed `REPORT: no GATE header` line and is treated as `stages=3c`, never a silent skip. Phase 1 fans out; phase 2 is `Test-GridDisposition` alone, on the reports phase 1 produced, with the interim band verdict `3c-band-verdict.json` written at the join; phase 3 cuts the figure sheet, and only on a green band over an unmoved spine. The runner threads its OWN resolved profile into the gates that read one, because a gate refusing in a real run for want of a parameter the runner already holds is a self-inflicted gap.

- `Run-SpineGates -Stage 1|2|3c`. Stages 1 and 2 are the **seed** bands over the corpus, the contract and the registry; membership comes from each gate's own `# GATE: stages=...` header, nothing is hand-listed, and a stage no script declares is **REFUSED (exit 2)** with a `<stage>-results.json` recording FAIL and no member - never a pass. Every arm of a seed run is labelled `seed`, and a seed result never counts as 3c evidence.
- **`-Only` is a partial run and cannot stand as evidence.** It writes `3c-results.partial.json` and `3c-band-verdict.partial.json`, leaves `3c-results.json` and `3c-band-verdict.json` untouched, never cuts the figure sheet, and **exits 3 even when every selected gate passed**. Use it to iterate on one gate's findings; a round is closed only by a full band.
- A member that exits 0 while a **blocking arm** on its `ARMS:` line neither ran nor was declared not applicable is recorded **FAIL** ("arm not run: `<name>`") and the band fails naming the member and the arm.
- `Test-SubSection -All` exiting 0 while any per-file `gate.json` it wrote says fail (or was never written, or disagrees with the wrapper) **fails by name**, in the wrapper itself and again in the runner.
...
- Exit codes: 0 pass, 1 a member failed / was refused / timed out / is unavailable, 2 a usage error (no unit extract at 3c, or a stage no script declares), 3 a partial run, 4 the self-test failed.

Any row in the table below that no gate's header binds to `3c` is performed by nobody at 3c, and the record must say so. Measured on the reference build, 4 Sep 2026: **band wall clock 157 s against 741 s run one after another, 21 per cent of the sum**, the slowest member being the sub-section wrapper at 153 s - so the band costs what its slowest gate costs, and adding the ten new gates cost 28 seconds of wall clock rather than 410.
...
| `Test-Readability`, count-based arm | **Status: NOT YET IMPLEMENTED** - performed today by: nobody before the render. `Test-Readability` takes an unpacked `.docx` (`-WorkDir`, `-Part`) and no wrapper feeds it the spine's prose fields, so first readability detection is still the Stage 4b run. Specified: paragraph length and real-lists, on the spine's **prose fields**. This is an ADDED run, not a moved one: the Stage 4b run on the rendered document is unchanged and still blocks. | The spine has no prompt paragraphs to strip, so the confound the old gate scripted around does not exist here. |
...
```powershell
# The sheet is cut by the BAND, in its phase 3, and only on a band that passed.
# This is the only route that produces evidence.
& "$SkillDir\scripts\Run-SpineGates.ps1" -BuildDir $out       # phase 3 writes figure-sheet.txt

# A re-cut after a spine edit takes the same proof and refuses without it.
& "$SkillDir\scripts\New-FigureSheet.ps1" -BuildDir $out -BandResults "$out\3c-results.json"
```

**The sheet cannot be cut from a spine that failed or moved.** `New-FigureSheet` takes `-BandResults` (default `3c-results.json`; the runner hands it the interim `3c-band-verdict.json` it writes at its own join, before phase 3) and **exits 2 with no file written** unless that file says verdict `PASS`, `partial` false, `exitCode` 0 **and** its `spineFingerprint` equals the fingerprint of the spine on disk right now. The refusal names the failed members and both fingerprints. Every sheet carries `BAND-VERDICT`, `BAND-RESULTS-SHA256` and `BAND-RAN-AT`, so a later reader can tell a proven cut from a forced one without trusting a note. `-Force` requires `-ForceReason` and stamps `BAND-VERDICT: FAIL`, so a forced sheet can never reach delivery. A spine with **no visual entries** is a `CHECK-SET EMPTY` refusal, not a sheet reporting its own emptiness. On the reference build the runner refused the cut and the documented direct call cut the sheet 19 seconds later from that same failed spine; that route no longer exists.

**It is generated, never written by hand, and it is stamped with the fingerprint of the spine it was cut from.** Generated, because a hand-assembled transcript drifts from the spine the moment either is edited - one build held its diagram content as hand-typed copies inside a spec-writer, and three rounds of spine corrections never touched them. Stamped, because **Stage 7 edits the spine**: a sheet nobody regenerated hands every later reviewer figure content the document no longer has, while the ledger records that the figures were read. `Test-StageLedger` recomputes the fingerprint and **blocks delivery** when it does not match, so **regenerate the sheet at the end of every Stage 7 round** - and the re-cut needs a **current** 3c band: re-run `Run-SpineGates` first, because `New-FigureSheet` compares the sheet's spine against the band results and refuses a cut the band has not judged - and re-run any review that was handed the old one.

**The sheet is proved by three things, not one.** It must carry **`BAND-VERDICT: PASS`** - the 3c band's own stamp - so a sheet cut by the direct `New-FigureSheet.ps1` command from a failed spine, or forced with `-Force` (which stamps `BAND-VERDICT FAIL`), can never reach delivery. Its `SPINE-FINGERPRINT` stamp is compared through `Test-GateFingerprintVersion`, which distinguishes "the fingerprint format changed, re-cut" from "the spine moved", and it accepts the `v2:<32 hex>` stamps `Get-SpineFingerprint` now returns. **An empty expected fingerprint is a problem, never a match.** And a Stage 5, 6 or 7d record written while the rendered extract's `FIGURES:` stamp still shows unresolved artwork prompt blocks must NAME the sheet it was read with (`-FigureSheet`); the sheet it names is checked then and there.
...
# and PRINTED. The member list is New-GateInvocationPlan in Run-Gates.ps1, not a copy here;
# the leakage sweep is REFUSED without unit_extract.md rather than allowed to degrade;
# the figure registry runs on sources AND on extracts the runner derives itself.
...
Stage 4 runs `scripts\Run-Gates.ps1 -BuildDir <dir>` (no `-AfterArtwork`). Same plan, same phases, same member list - see `New-GateInvocationPlan`. It writes `4-results.json`. Readability measures a copy with the artwork prompt paragraphs stripped, and the placed-artwork gate and the two rendered arms are recorded not-applicable by name.
...
**Switches:** `-AllowPartial` (threaded to `Test-GuideRules` and `Test-DeckRules` and printed), `-SpineDir`, `-RequireFresh` (Stage 8 only: runs no gate and writes nothing; refuses exit 2 when a delivered artefact was written after the results file that judges it, when its sha256 no longer matches, or when that file's verdict is not pass) and `-SkipPlantChannel`.
...
**A same-brand build runs the brand step too, and there is no build-local repaint script.** `Set-DeckBrand` and `Set-GuideBrand` no longer no-op when the target brand equals the pack's `templates.brand`: they run the palette step as **NORMALISATION** across every `.xml` and `.rels` part of the package (notesMasters included), anchored on the colour attribute, and they sweep the template's own docProps ancestry. Measured on the approved MVC deck template: straight off disk it fails `Check-Identity` with **21 crossover hits** - 18 legacy hexes across ten slide parts plus three docProps strings naming a different institute - and after the normalisation step it passes, exit 0. That round is what `Fix-DeckPalette` was written for, and it has no reason to exist.

Both functions now take `-TemplateBrand`, and it is **blocking**: with no value they throw naming `templates.brand`, because without it a normalisation cannot be told from a crossover.
...
**Stage 6 - clean-room compliance audit. The stage this skill exists to survive.** Reviewers with **none of the build context** - the guide, the deck, the assessment pack, `references/audit-checklist.md`, and **their own independent extraction of the unit, already prepared in the background since Stage 0**. Use the prompt in the checklist verbatim. **The audit is split by topic, and the reason is measured, not stylistic:** one reviewer given everything holds about 350K tokens, beyond one context, so the "single reviewer" was always a chain of partial reads - which is why six rounds on one build each found a different defect class. `scripts\New-ReviewPack.ps1` cuts one pack per topic plus the cross-document pack(s): a topic reviewer gets the two learner-facing tools whole, the unit extract, its figure-sheet slice, the allow-list with every written reason, and from the assessor guides ONLY the regions for the tasks its topic prepares, taken from the contract's questionMap and the guides' own Contents headings (on the reference build that cut each topic's assessor material from 348 KB to about 20 KB, and every pack from 200-219K tokens to 121-144K). The pack text is resolved the way every gate resolves it, through `Get-GateCorpusDir` (an explicit `-PackDir`, then `corpus\`, then the pre-corpus locations), so the cutter and the gates can never read two different extractions of one pack; a given `-PackDir` that holds no extract is refused by name rather than silently swapped. **A reference kind that resolves to no learner document fails the cut**, naming the kind and every learner document searched, rather than handing each topic reviewer NOT FOUND banners for benchmarks that do exist. Cross-document consistency stays mandatory and belongs to the cross-document reviewer, which works from `scripts\Get-ClaimsDigest.ps1` (every numeral-with-unit, locked term, citation, question reference and scenario time, with its location) plus the figure sheet and registry, and splits into a values half and a references half when one pack would exceed the 180K budget. Cross-topic mirroring is the mirror gate's job over the whole spine, not a human's. **`scripts\Merge-AuditFindings.ps1` merges the findings, and it is a script, never an agent** - a summarising merger would be a ninth reviewer with sight of the other eight, which the isolation rule forbids; it reads the expected reviewer set from the pack manifest and **refuses by name any directory the manifest does not expect** - a stale `topic9` from an earlier nine-topic cut, or a previous merge's own output - as it refuses a missing `findings.json`, a `findings.json` whose `reviewer` is `merger`, and one whose reviewer does not match its directory. It unions coverage claims against the unit extract and raises any KE or PE item no topic claims to teach; **a coverage claim whose `anchors[]` is empty is listed and counts as no coverage**, because a claim you cannot anchor is a claim you do not make. **The KE/PE check-set is derived from `contract.json` `keMap` unioned with the identifiers in the unit extract, and the merge is refused naming both when neither yields an item** - the reference build's extract writes its knowledge evidence as prose and carries no `KE1`-shaped identifier at all, on which this arm used to exit 2 and check nothing. It validates every `where.artefact` against a closed vocabulary (the guide and the deck, plus `pack` and each `packDocs` name) and refuses a misspelt one; **a finding against the pack is carried in full and reported UPSTREAM, and does not floor the learner guide's verdict**. It groups duplicates on anchor and class, keeps the worse risk as the finding and **still emits every other copy verbatim with `sameAnchorAs`**, and lets the worst verdict win per artefact. **The merge writes its own Stage 6 record** through `Add-StageRecord` - round counted from the ledger, the sha256 of the merged `findings.json` in the note - so the ledger records the audit by machine, not from memory; `-BuildDir` names the build (it defaults to the manifest's `buildDir`), a build with no ledger is refused, and `-NoLedger` marks a scratch merge in the open. The single-reviewer mode for a unit small enough to fit is `-FullPack` on the CUTTER, `scripts\New-ReviewPack.ps1`; the merger has no such switch. The merge is run as:

```
scripts\Merge-AuditFindings.ps1 -ReviewDir <build>\review -UnitExtract <build>\unit_extract.md -OutPath <build>\review\merged\findings.json -MarkdownPath <build>\review\merged\merged_audit.md -BuildDir <build>
```

`-BuildDir` may be omitted when `manifest.json` carries `buildDir`. Omitting both, without `-NoLedger`, is refused: the merged findings ARE the Stage 6 record and the ledger must receive them.
...
`scripts\Test-Finding.ps1` is the arbiter, and it **refuses rather than reports when a blocking input is absent**: no corpus resolves (even with `-PackDir`, which is extra source text and never a substitute), or no spine file is found - either exits 2 naming the input, because a finding "tested" against zero documents comes back UNREFUTED and reads as a clearance. A leak finding whose mirror gate could not run is left **UNCHECKED**, never UNREFUTED. The value search is boundary-anchored in both arms, so a finding valued `7.5 L` is not refuted by a corpus that says `17.5 L`.
...
6. **Regenerate the figure sheet** - which step 5's re-run of the 3c band does, in its phase 3, because step 1 to 3 edited the spine the sheet was cut from and it is what every later reviewer reads figure content from. A direct `New-FigureSheet.ps1 -BandResults <3c-results.json>` re-cut is available and **refuses a spine the band did not pass**, so there is no route from a failed spine to a sheet. Stage 8 blocks on a sheet whose stamped spine fingerprint no longer matches, and on one whose `BAND-VERDICT` is not `PASS`.
...
## Stage 7b - Place the artwork (position unchanged: after Stage 7's re-render, before delivery)
...
**Build the Contents now, not at Stage 8.**

```powershell
& "$SkillDir\scripts\Finish-Documents.ps1" -UpdateContents -Guide $guidePath
```

Updating a table of contents **saves the document**. Doing it at Stage 8 rewrote every delivered `.docx` after the gate that judged it, and nothing downstream could tell a Contents rebuild from a content edit, because the evidence is the same - a newer file. This is the **last write** to the guide; 7c then gates the document as it will ship.
...
Stage 7c runs `scripts\Run-Gates.ps1 -BuildDir <dir> -AfterArtwork`. **The member list is not repeated here: it is `New-GateInvocationPlan` in `scripts\Run-Gates.ps1`**, and every gate whose own `# GATE:` header names stage 7c must appear in it - the runner FAILS naming any that does not. The run writes `7c-results.json`; a run without `-AfterArtwork` is Stage 4 and writes `4-results.json`, so a Stage 4 run can never overwrite the 7c evidence. Entries the stage cannot run are recorded by name and reason in the results file's `partial[]`, never dropped: `placed` (Check-Figures) and the two rendered arms are `not-applicable` before artwork, and `Assert-ChannelDisposition` is `not-implemented`. The runner REFUSES (exit 2) and runs nothing when the spine's v2 fingerprint differs from the one stamped in the newest FULL `3c-results.json`, naming both fingerprints; a `-Only` run's `3c-results.partial.json` is not the band and does not satisfy this.

What that plan covers, so a reader can see the claim rather than take it:
...
- `scripts\Test-GridDisposition.ps1`, `scripts\Check-ShapeMirror.ps1` and `scripts\Check-RowCoverage.ps1 -Whole` - the same three that block at 3c, re-run here because placement is a mutation. The disposition gate reads all three producer reports and refuses (exit 2, naming the file) any that is absent, unstamped, cut from a different spine, cut in file mode or generated before the run started, so a 7c verdict cannot be cut from a stale spine report; the runner threads its own start time as `-NotBefore` and plans the disposition in a phase after the three producers.
