...
It also **refuses by name any directory the manifest does not expect** - a stale `topic9` from an earlier nine-topic cut, or a previous merge's own output - as it refuses a `findings.json` whose `reviewer` is `merger` and one whose reviewer does not match its directory. **A coverage claim whose `anchors[]` is empty is listed and counts as no coverage**, because a claim you cannot anchor is a claim you do not make. **The KE/PE check-set is derived from `contract.json` `keMap` unioned with the identifiers in the unit extract, and the merge is refused naming both when neither yields an item** - the reference build's extract writes its knowledge evidence as prose and carries no `KE1`-shaped identifier at all, on which this arm used to exit 2 and check nothing. Every `where.artefact` is validated against a closed vocabulary (the guide and the deck, plus `pack` and each `packDocs` name) and a misspelt one is refused; **a finding against the pack is carried in full and reported UPSTREAM, and does not floor the learner guide's verdict**. Duplicates are grouped on anchor and class, the worse risk is kept as the finding, and **every other copy is still emitted verbatim with `sameAnchorAs`**.

**The merge writes its own Stage 6 ledger record** through `Add-StageRecord` - round counted from the ledger, the sha256 of the merged `findings.json` in the note - so the ledger records the audit by machine, not from memory:
...
`-BuildDir` may be omitted when `manifest.json` carries `buildDir`. Omitting both, without `-NoLedger`, is refused: the merged findings ARE the Stage 6 record and the ledger must receive them. A build with no ledger is refused; `-NoLedger` marks a scratch merge in the open. The single-reviewer mode is `-FullPack` on the CUTTER, `scripts\New-ReviewPack.ps1`; the merger has no such switch.

**The cutter resolves the pack text the way every gate resolves it**, through `Get-GateCorpusDir` (an explicit `-PackDir`, then `corpus\`, then the pre-corpus locations), so the cutter and the gates can never read two different extractions of one pack; a given `-PackDir` that holds no extract is refused by name rather than silently swapped. **A reference kind that resolves to no learner document fails the cut**, naming the kind and every learner document searched, rather than handing each topic reviewer NOT FOUND banners for benchmarks that do exist.
